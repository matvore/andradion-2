...
namespace NGameLib2
{
  ...
  CBob::~CBob()
  {
    WriteLog("CBob Destructor called for Bob at %8x" LogArg((DWORD)this));
    if(NULL != this->data)
      {
  ...
  int CBob::GetHeight()
  {
    ...
    int h;
    this->GetSize(w,h);
    WriteLog("CBob::GetHeight: we found out that the height was %d, but also that the width was %d" LogArg(h) LogArg(w));
    return h;
  }
  ...
  void CBob::GetSize(int& width,int& height)
  {
    ...
    WriteLog("CBob::GetSize: about to get surface description so we can get the size of the surface");
		
    this->data->GetSurfaceDesc(&ddsd);
  ...
  void CBob::Create(LPDIRECTDRAWSURFACE2 parent)
  {
    ...
    MemoryAllocFunction(
			res = TryAndReport(parent->QueryInterface(IID_IDirectDrawSurface,(void **)&version1_of_parent)),
			sizeof(int),
			FAILED(res)
			);
    ...
    MemoryAllocFunction(
			res = TryAndReport(CGraphics::DirectDraw()->DuplicateSurface(version1_of_parent,&version1_of_data)),
    ...
    WriteLog("release the unneeded versions of both . . .");
    TryAndReport(version1_of_data->Release());
    TryAndReport(version1_of_parent->Release());
  ...
  CBob::CBob(int w,int h,DWORD or,DWORD and) : data(NULL)
  {
    WriteLog("CBob blank surface constructor called for Bob at address %8x" LogArg((DWORD)this));
    this->Create(w,h,or,and);
  }

  CBob::CBob(HBITMAP bmp,DWORD or,DWORD and) : data(NULL)
  {
    ...
    // create the surface
    this->Create(bmp_info.bmWidth,bmp_info.bmHeight,or,and);
  ...
  void CBob::Create(int w,int h,DWORD or,DWORD and)
  {
  ...
  {
    WriteLog("CBob::IsCreated called for Bob at memory address %8d with a surface at memory address %8d" LogArg((DWORD)this) LogArg((DWORD)this->data));
