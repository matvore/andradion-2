...
Gfx::SurfaceLock::~SurfaceLock() {
  if (surface) {
    ...
    WriteLog("Destroying fake surface pointer");
    delete [] surface_ptr;
  }
}

Gfx::Surface::~Surface() {
  WriteLog("Destroying surface %x" LogArg((int)this));
...
(HINSTANCE hInst, LPCTSTR res_name)
  ...
  WriteLog("Created new BitmapSurfaceFiller: %x" LogArg((int)this));
}
...
HBITMAP Gfx::BitmapSurfaceFiller::LoadImage(int wanted_width,
  ...
  WriteLog("BitmapSurfaceFiller::LoadImage called with arguments (%d, %d)"
           LogArg(wanted_width) LogArg(wanted_height));
  ...
  if (width < 0 && !wanted_width && !wanted_height) {
    if (!GetObject(res, sizeof(bmp_info), &bmp_info)) {
      WriteLog("Could not get width and height of bitmap");
      width = 1;
      height = 1;
    } else {
  ...
  WriteLog("Returning from BitmapSurfaceFiller::LoadImage()");
...
void Gfx::BitmapSurfaceFiller::Fill(BYTE *surf_ptr, int surf_pitch,
  ...
  WriteLog("Refilling surface %x with bitmap" LogArg(int(surface)));
...
(HWND hWnd, int mode_width, int mode_height, int refresh_rate,
 ...
 int virtual_buffer_height, int virtual_buffer_count) 
  ...
  WriteLog("Setting cooperative level and display mode");
  ...
  WriteLog("Creating primary surface(s)");
  DoOrFail(__LINE__, direct_draw->CreateSurface
  ...
  WriteLog("Gfx constructor finishing");
}
...
void Gfx::SetPalette(PALETTEENTRY *pe, int entry_count,
  ...
  WriteLog("Getting/Creating DirectDraw palette");
  ...
  if (!visual_effect) {
    WriteLog("Refilling surfaces");
    RefillSurfaces();
  }

  WriteLog("SetPalette method returning");
}
...
void Gfx::DoOrFail(int line, HRESULT hr) throw(Exception) {
  if (FAILED(hr)) {
    WriteLog("Operation on line %d failed with code %x"
             LogArg(line) LogArg(hr));
...
AutoComPtr<IDirectDraw2> Gfx::CreateDirectDraw() {
  ...
  WriteLog("Creating DirectDraw interface");
