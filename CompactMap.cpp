...
inline void PutCompactMapRowNoLeftRightClip
(
...
inline void PutCompactMapRowRightClipOnly
(
 ...
 )
{
  ...
  for(VCTR_RUN::iterator run_i = row.begin(); run_i != row.end(); run_i++)
      ...
      if(x + bytes_to_copy > max_x)
	{
      ...
      memcpy((void *)(lpSurface+x),(const void *)&run_i->second[0],bytes_to_copy);
    }
}

inline void PutCompactMapRowLeftClipOnly
(
      ...
      if(x + size > min_x)
	{
	  ...
	  if(first_byte_to_copy >= 0)
		...
		(
		 ...
		 (const void *)&run_i->second[first_byte_to_copy],
  ...
  for(;run_i != row.end(); run_i++)
    {
      memcpy(
	     (void *)(lpSurface+start_x+run_i->first),
	     (const void *)&run_i->second[0],
	     run_i->second.size()
	     );
    }
}

inline void PutCompactMapRowLeftRightClip
(
...
// putting entire maps:
inline void PutCompactMapStep2of2NoLeftRightClip
(
...
inline void PutCompactMapStep2of2RightClipOnly
(
...
inline void PutCompactMapStep2of2LeftClipOnly
(
...
inline void PutCompactMapStep2of2LeftRightClip
(
...
CCompactMap::CCompactMap() : CCertifiable()
{
  //WriteLog("Default CCompactMap constructor called for map at memory address %8x" LogArg((DWORD)this));
}
...
int CCompactMap::Certify()
{
  ...
  for
     ...
     )
    {
      ...
      minmax_i->second = (long)2147483648uL;
    }
  this->min_max_x_offsets.first = 2147483647L;
  this->min_max_x_offsets.second = (long)2147483648uL;
