...
const int GUNTRAILPENSTYLES[] = {PS_SOLID,PS_DASH,PS_DASHDOT,PS_DOT,PS_DASHDOTDOT};
...
void CFire::Setup(FIXEDNUM sx_,FIXEDNUM sy_,int direction_,int type_,bool remotely_generated_)
{
  FIXEDNUM mx;
  FIXEDNUM my;
...
void CFire::Logic() // returns true if it can be safely deleted
{
  ...
  if(FIRESTATE_GOING == this->state)
      ...
      do
	{
	  if(WEAPON_PISTOL == this->type)
	    {
	      if(proj_speed_x > 0)
		{
		  hypothetical.second.x = min(dest_x,(int)hypothetical.first.x+proj_speed_x);
		}
	      else
		{
		  hypothetical.second.x = max(dest_x,(int)hypothetical.first.x+proj_speed_x);
		}
	      if(proj_speed_y > 0)
		{
		  hypothetical.second.y = min(dest_y,(int)hypothetical.first.y+proj_speed_y);
		}	
	      else
		{
		  hypothetical.second.y = max(dest_y,(int)hypothetical.first.y+proj_speed_y);
		}
	    }
	  else
	    {
	  ...
	  if(WEAPON_BAZOOKA != this->type)
	      ...
	      if(FIRECOLLIDES_HERO == coll)
		{
		  ...
		  this->state = WEAPON_PISTOL != this->type ? FIRESTATE_FIRSTFRAMETODRAW : FIRESTATE_INACTIVE;
		  return;
		}
      ...
      for(VCTR_INT::const_iterator iterate = collex.begin(); iterate != collex.end(); iterate++)
	{
	  ...
	  else if(GLUenemies.size() > *iterate)
	    {
	      if(false == NetInGame())
		{
