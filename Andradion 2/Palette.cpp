...
const MENUPALETTE_BASECOLORS = 249;

FIXEDNUM current_brightness_factor;
static LPDIRECTDRAWPALETTE pal = NULL;
static LPDIRECTDRAWSURFACE2 surf;
static bool need_to_apply_brightness_factor;

void PalInitializeWithMenuPalette(CGraphics& gr)
{
	// make sure we are not initialized
	assert(NULL == pal);
	pal = gr.CreatePalette(false);
	gr.Buffer(0)->SetPalette(pal);
	current_brightness_factor = Fixed(1);
	...
	CColor256::ChangePalette(pe,256);
	pal->SetEntries(0,0,256,pe);
...
// this function fills target with a random
//  rgb element value.  Should be called three
//  times for each palette entry.  Returns 1 if
//  target has become zero, or returns 0 otherwise
...
{
	need_to_apply_brightness_factor = false;
	if(false == ignore_data)
	{
		// make sure we are not initialized
		assert(NULL == pal);
		pal = gr.CreatePalette(false);
	...
	if(false == ignore_data)
	{
		CColor256::ChangePalette(pe.begin(),256);
		pal->SetEntries(0,0,256,pe.begin());
...
{
	assert(NULL != pal);
	surf->SetPalette(NULL);
	TryAndReport(pal->Release());
	pal = NULL;
}
...
void PalSetBrightnessFactor(FIXEDNUM new_brightness_factor)
{
	...
	if(current_brightness_factor != new_brightness_factor)
	{
		// record our new brightness
		current_brightness_factor = new_brightness_factor;
		need_to_apply_brightness_factor = true;
	}
}

void PalApplyBrightnessFactor()
{
	if(false == need_to_apply_brightness_factor)
	{
		return;
	}
	...
	// get the palette from the gdi palette and set
	//  all superfluous entries to zeros 
	PALETTEENTRY pe[256];
	int num_copied = GetPaletteEntries(CColor256::GetGDIPalette(),0,256,pe);
	...
	{
		ScaleColor(pe[i].peRed,current_brightness_factor);
		ScaleColor(pe[i].peGreen,current_brightness_factor);
		ScaleColor(pe[i].peBlue,current_brightness_factor);
	}

	pal->SetEntries(0,0,num_copied,pe);
}
