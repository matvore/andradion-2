...
GfxBasic::GfxBasic(HWND hWnd, int mode_width, int mode_height, int refresh_rate,
  ...
  for (int y = 0, i = 0; y < grid_height; y++) {
    for (int x = 0; x < grid_width; x++, i++) {
      ...
      WriteLog("Clip area for virtual buffer %dx%d created: [%d, %d; %d, %d]"
               LogArg(x) LogArg(y) LogArg(left) LogArg(top)
               LogArg(left + virtual_buffer_width)
               LogArg(top + virtual_buffer_height));
    }
  }
...
(int w, int h, auto_ptr<Gfx::SurfaceFiller> filler) {
  ...
  WriteLog("Changing filler of new surface");
  result->ChangeFiller(filler);

  WriteLog("Filling new surface for first time");
  result->Refill();

  WriteLog("GfxBasic::CreateSurface() returning");
  return result;
}
