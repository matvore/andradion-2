...
IToken(HEALTH_PACK) // set the constant
{
	// just set the state	
...
void CHealthPack::Draw(void)
{
	...
	//  parency
	gr->Put(
		CMedia::Media().Bitmap(BHEALTH),
		true
	);
}

void CHealthPack::Logic(unsigned char keyb[KEYBOARD_BUFFER_SIZE])
{
