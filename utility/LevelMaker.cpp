...
const int CHARACTERS[NUM_CURSORS] = {
  ...
  IDB_GENERICSALLY,
  IDB_GENERICMILTON,
  IDB_GENERICEVILTURNER,
  IDB_GENERICPISTOL,
  IDB_GENERICMACHINEGUN,
  IDB_GENERICBAZOOKA,
  IDB_GENERICHEALTH
};
...
const char LEVEL_NAMES[][NUM_LEVELS] =
  {"1_", "2a", "2b", "3a", "3b", "4_", "5_", "6a", "7a", "6b", "7b"};
...
static void Put(int x,int y) {
  ...
  // Never allow more than one Mr. Turner.
  if(0 == current_cursor) {locations[0].clear();}
...
LRESULT WINAPI WindowProc(HWND hWnd, UINT Msg, WPARAM wParam,
  ...
  case WM_LBUTTONUP:
    ...
    return 0;
  case WM_RBUTTONUP:
    current_cursor = (current_cursor + 1) % NUM_CURSORS;
...
int APIENTRY WinMain(HINSTANCE ourInstance,HINSTANCE,LPSTR,int) {
  MessageBox(
	     ...
	     "Alt+F4: leave\n\n"
	     "Levels are saved AUTOMATICALLY, whenever the current one is changed or the application ends!",
	     "Instructions",
	     MB_ICONINFORMATION);
  ...
  if(0 == RegisterClassEx(&winclass)) {return 0;}

  hWnd =
    CreateWindow("Andradion 2 Level Maker","Andradion 2 Level Maker",WS_POPUP|WS_VISIBLE,
